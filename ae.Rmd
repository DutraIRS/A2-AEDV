...
# Desconsidera as linhas duplicadas.
...
O gráfico anterior ficou de difícil visualização, porque a variável tipo de vinho possui muitos valores que se confundem no aspecto cor. Por isso, resolvi retirar essa variável por enquanto.
...
Abaixo eu importei uma biblioteca que me permite relacionar uma paleta de cores a uma variável e tornei corpo uma variável não contínua para evitar confusão na visualização.
...
Voltando ao gráfico com cores, preparei a variável acidez para caber no parâmetro forma e escolhi manualmente três formatos adequados aos seus valores. Depois eu modifiquei as formas para destacar a cor de cada ponto.
...
Dá pra ter uma noção do que acontece, mas criando um gráfico para cada tipo, poderemos ver melhor como as variáveis se comportam.
```{r}
...
```
Bom, dá pra ver que a maioria dos vinhos é recente, e quando temos um vinho mais velho, ele costuma ser minimamente bem classificado. Mas entre os vinhos novos, há diversas classificações.

#Comparando preço e classificação de cada tipo de vinho.

Essa sequência de gráficos pode ser útil para relacionar outras variáveis. A seguir, plotamos um facetamento por tipo e um gráfico de cada tipo. Nos gráficos cruzamos preço com classificação e adicionamos a variável número de avaliações na cor e no tamanho (pra ter destaque).
```{r}
...
Ser velho não significa ser bom. Ser caro também não significa ser bom. A partir de mil euros, o preço para de influenciar na qualidade. Os bem classificados possuem muito corpo e acidez, mas possuir muito corpo e acidez não define sua classificação. Com excessão do Sparkling, do Sherry, Sauvignon Blanc, Pedro Ximenez e Montsant Red, todos os tipos de uva podem produzir vinhos ruins. Mas apenas Toro Red e Tempranillo produzem os que tiveram melhor avaliação. Dentre os melhores, o mais barato é o Vina El Pison 2018, de uva Tempranillo, e que custa 313,50 euros; De acordo com o site vivino, ele é vinho top 1% do mundo. Seu corpo é quatro, e sua acidez é dois. O melhor abaixo de cem euros é o Regina Vides Ribera del Duero 1998, e sua avaliação é 4.7. Dentre os melhores vinhos, o Don PX Convento Seleccion 1946 é o primeiro com um grande número de avaliações (398).
